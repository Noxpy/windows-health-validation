<#
.SYNOPSIS
    Executes CHKDSK with /F and /R safely, with reboot awareness, structured logging,
    explicit scheduling validation, and timeout protection.

.DESCRIPTION
    Runs CHKDSK /F /R on the specified drive after checking for:
...
$LogFile   = Join-Path $LogRoot "CHKDSK_FR_Option_$Timestamp.log"
$Drive     = "C:"
$TimeoutMs = 180000  # 3 minutes for combined /F /R

if (-not (Test-Path $LogRoot)) { New-Item -Path $LogRoot -ItemType Directory -Force | Out-Null }
"==== CHKDSK /F /R START ====" | Out-File $LogFile -Append
...
# ---------------- Execute CHKDSK /F /R with Timeout ----------------
Write-Host "Scheduling CHKDSK /F /R on $Drive..." -ForegroundColor Yellow

$job = Start-Job -ScriptBlock {
    param($drive)
    cmd.exe /c "echo Y|chkdsk $drive /F /R 2>&1" | Out-String
} -ArgumentList $Drive
...
if ($ScheduledAfter) {
    $Result  = 2
    $Message = "CHKDSK /F /R successfully scheduled for next reboot."
}
...
elseif ($logText -match $cleanPattern) {
    ...
    $Result  = 1
    $Message = "CHKDSK /F /R completed with warnings or ambiguous output."
}
...
"==== CHKDSK /F /R END ====" | Out-File $LogFile -Append
