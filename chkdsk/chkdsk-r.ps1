<#
.SYNOPSIS
    Executes CHKDSK with /R (recover) safely, with reboot awareness, structured logging,
    explicit scheduling validation, and timeout protection.

.DESCRIPTION
    Runs CHKDSK /R on the specified drive after checking for:
...
$LogFile   = Join-Path $LogRoot "CHKDSK_R_Option_$Timestamp.log"
$Drive     = "C:"
$TimeoutMs = 120000  # 2 minutes for /R, may need longer

if (-not (Test-Path $LogRoot)) { New-Item -Path $LogRoot -ItemType Directory -Force | Out-Null }
"==== CHKDSK /R START ====" | Out-File $LogFile -Append
...
# ---------------- Execute CHKDSK /R with Timeout ----------------
Write-Host "Scheduling CHKDSK /R on $Drive..." -ForegroundColor Yellow

$job = Start-Job -ScriptBlock {
    param($drive)
    cmd.exe /c "echo Y|chkdsk $drive /R 2>&1" | Out-String
} -ArgumentList $Drive
...
if ($ScheduledAfter) {
    $Result  = 2
    $Message = "CHKDSK /R successfully scheduled for next reboot."
}
...
elseif ($logText -match $cleanPattern) {
    ...
    $Result  = 1
    $Message = "CHKDSK /R completed with warnings or ambiguous output."
}
...
"==== CHKDSK /R END ====" | Out-File $LogFile -Append
